...
// ARKA CUSTOMIZATION: server ID/Rendezvous and public key are injected at
// build time via the `RENDEZVOUS_SERVER` / `RS_PUB_KEY` environment variables
// (read by `option_env!`). If they are not set (or set empty) at compile time,
// the fallback values below are used. Original RustDesk defaults were:
